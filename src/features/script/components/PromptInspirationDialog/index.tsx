...
import { PROMPT_TEMPLATE_CATEGORIES, type PromptTemplate } from '@/features/script/prompt/templates';
...
export function PromptInspirationDialog({ isOpen, onOpenChange, onPick }: PromptInspirationDialogProps) {
    ...
    const categoryLabels = {
        scenes: t('Scenes'),
        camera: t('Camera moves'),
        style: t('Style <and> Light')
    };

    const category = PROMPT_TEMPLATE_CATEGORIES.find((c) => c.id === activeCategory) ?? PROMPT_TEMPLATE_CATEGORIES[0];

    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className='max-h-[80vh] overflow-y-auto border-white/20 bg-black text-white sm:max-w-[560px]'>
                <DialogHeader>
                    <DialogTitle className='text-white'>{t('Prompt Inspiration')}</DialogTitle>
                    <DialogDescription className='text-white/60'>
                        {t('Scene templates replace your prompt<semi> camera and style phrases are appended to it')}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className='flex gap-2'>
                    {PROMPT_TEMPLATE_CATEGORIES.map((c) => (
                        <button
                            ...
                            {categoryLabels[c.id as keyof typeof categoryLabels] ?? c.label}
                        </button>
                    ))}
                </div>
                    ...
                    <div className='space-y-2'>
                        {category.templates.map((t) => (
                            <button
                                ...
                                <p className='text-sm font-medium text-white'>{templateLabel(t.label)}</p>
