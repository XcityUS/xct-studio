...
function portraitRank(item: VideoPortrait): number {
    return item.status === 'Processing' ? 0 : 1;
}

function portraitVersion(item: VideoPortrait): number {
...
export function mergeDocs(local: HistoryDoc, remote: HistoryDoc): HistoryDoc {
    ...
    for (const portrait of [...remote.portraits, ...local.portraits]) {
        if (tombstoned.has(portrait.assetId)) continue;
        const existing = portraitByAssetId.get(portrait.assetId);
        if (
            !existing ||
            portraitRank(portrait) > portraitRank(existing) ||
            (portraitRank(portrait) === portraitRank(existing) &&
                portraitVersion(portrait) >= portraitVersion(existing))
        ) {
            portraitByAssetId.set(portrait.assetId, portrait);
        }
    }
