...
import {
    ...
    EXPORT_FORMATS,
    NONE_BGM_VALUE,
    ORIGINAL_EXPORT_FORMAT
} from './constants';
import { downloadBlob, downloadSrt, downloadXml, fetchBgmBlob } from './media';
import type { AssemblyEditorProps, ClipRow, ExportFormatId } from './types';
import {
    displayNameForAsset,
    displayNameForClip,
    exportBaseName,
...
import { getVideoDuration } from '@/features/assets/media/thumbnail';
import { parseSize, pixelDimensions } from '@/shared/config/seedance';
import {
    CaptionBurnUnavailableError,
    assembleClips,
    type AssembleClip,
    type AssembleOptions
} from '@/features/post-production/assembly/client';
import { buildFcp7Xml, type Fcp7XmlClip } from '@/features/post-production/export/nle';
                        ...
                        <div className='grid gap-3 md:grid-cols-[minmax(0,1fr)_16rem] md:items-end'>
                            <div className='flex items-start gap-2'>
                                ...
                                <div>
                                    ...
                                    {selectedReframe && (
                                        <p className='mt-1 text-xs text-amber-200'>Reframing re-encodes the film.</p>
                                    )}
                                </div>
                            </div>
