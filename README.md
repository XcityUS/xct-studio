...
- **🎨 Text-to-image tab** _(optional)_ — Seedream models through the same gateway; generated images persist in the browser and can be sent straight back into image-to-video ("Animate").
...
| `TRANSCRIBE_MODEL`                   | for burned subtitles | TokenHub transcription model used to align generated speech with source dialogue before browser-side FFmpeg burn-in. Automatic subtitle modes generate a timed player track directly from the script and do not use transcription. It must support `/v1/audio/transcriptions`, `verbose_json`, and segment timestamps. |
