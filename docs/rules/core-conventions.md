...
- Automatic and burned-caption audio transcription calls the TokenHub `/v1/audio/transcriptions` gateway directly from the browser with the user's Xcity key after generation, matching the existing video-generation boundary. Studio aligns the selected script to returned word or segment timestamps and persists the resulting cue track; when transcription is unavailable it falls back to script-estimated timing without blocking video completion. Provider credentials remain inside LiteLLM; Studio owns only the public model alias, response validation, caption alignment, and selected delivery mode.
- Player and Studio-burned subtitle tracks keep one complete dialogue beat per timed cue and do not split cues by character count. The player and burn-in renderer own responsive wrapping inside the subtitle safe area.
