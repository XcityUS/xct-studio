...
| Studio or Seedream generated material      | Review-exempt for this product flow; retain generation provenance.                                  |
...
| Public figure                              | Complete likeness-rights evidence, offline approval/account allowlisting, and active Asset ID.      |
| Protected or well-known IP                 | Complete copyright authorization chain, offline approval/account allowlisting, and active Asset ID. |
...
- A multi-person asset records every recognizable person. Every person must have an applicable face-verification or offline authorization record before the Asset ID can become generation-ready.
...
- Passing Studio review only unlocks the Studio gate. Provider moderation may still reject a request, and the UI must preserve that distinction.
