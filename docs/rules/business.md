...
- Treat extracted characters and scenes as reviewable drafts, not generation-ready assets. Every extracted character and scene must have an asset binding before production can start; missing bindings are blocking findings for both batch and single-shot generation.
...
- Character and scene assets generated automatically for one short-drama project share one private AIGC group named after that project. Individual character or scene names remain asset names inside the group and must not create one provider group per image.
