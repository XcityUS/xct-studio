...
- Studio 不接数据库。新增业务 API、人物解析、云端保存、事务、Snapshot、Batch/Queue 均在 xcity-litellm；Studio 负责 UI 和薄代理。
- 项目入口只展示一个项目下拉；新建/编辑通过 Modal，不再重复显示项目名和常驻文本框。重名报错，不自动切到另一个项目。项目配置须补画幅、分辨率、水印、明确声音语言与字幕模式；首次明确确认语言，不继承 Normal 默认值。
...
本轮 UI 已实现：模式入口及刷新恢复、精简项目选择、项目独立配置、新建/编辑/删除、前端与数据层重名拒绝、自动拆分选择、重新拆分覆盖确认、标签页草稿保留、角色/场景/分镜结构化 Review、项目素材绑定、主要角色 Preflight、独立分镜编辑/复制、逐镜头时长/对白/字幕/连续来源、生成确认和浅色主题。AI 拆解改走 xcity-litellm 专用接口，Studio 保留同源薄代理。云端草稿、正式短剧数据模型、历史分区、后台 Batch/Queue 和刷新恢复尚未交付，不勾选 §44 完整验收项。
...
2026-09-10 边界修订：按用户要求，XCT Studio 不接数据库。此前在 Studio 新增的数据库、短剧业务 API 和队列执行器已撤回；这些能力由 xcity-litellm 后端负责，尚未迁移或部署。见 [后端边界说明](../architecture/short-drama-infrastructure.md)。以下验收项保持未完成，不将撤回的实验实现计作交付。
